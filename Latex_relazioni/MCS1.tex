...
\label{subsec:gs}
Il metodo di Gauss-Seidel adotta lo stesso schema generale di un metodo iterativo
stazionario, con una scelta specifica per la matrice di precondizionamento $P$. In
questo caso, $P$ è scelta come la parte \emph{triangolare inferiore} di $A$ (diagonale
inclusa), che denotiamo con $L$:
\begin{equation}
    P := L =
    \begin{pmatrix}
        a_{1,1} & 0       & \cdots & 0       \\
    ...
    \end{pmatrix},
\end{equation}
mentre la matrice $N$, definita come $N = P - A$, rappresenta la parte triangolare
superiore stretta di $A$ cambiata di segno.

\paragraph{Formula di aggiornamento.}
Ad ogni iterazione, l'algoritmo corregge l'approssimazione corrente ponendo
$x^{(k+1)} = x^{(k)} + y$, dove il vettore di correzione $y$ è la soluzione del
sistema triangolare inferiore:
\begin{equation}
    L\, y = r^{(k)}.
    \label{eq:gs_fwd}
\end{equation}
Il vettore $r^{(k)} = b - Ax^{(k)}$ è il residuo globale calcolato all'inizio
dell'iterazione $k$-esima. Il sistema~\eqref{eq:gs_fwd} viene risolto efficientemente
tramite l'algoritmo di \emph{sostituzione in avanti}, con un costo computazionale
$\mathcal{O}(n^2)$, evitando così l'inversione esplicita di $L$.

\paragraph{Forma componente per componente.}
L'algoritmo di sostituzione in avanti determina le componenti del vettore di
correzione $y$ riga per riga, sfruttando i valori $y_j$ già calcolati nella stessa
iterazione per gli indici $j < i$. La formula risultante è:
\begin{equation}
    y_i = \frac{1}{a_{ii}} \left( r_i^{(k)} - \sum_{j=1}^{i-1} a_{ij}\, y_j \right),
    \qquad i = 1,\dots,n.
    \label{eq:gs_comp_corr}
\end{equation}
Una volta determinate tutte le componenti di $y$, la nuova iterata viene aggiornata
tramite $x^{(k+1)} = x^{(k)} + y$. Questa formulazione è algebricamente equivalente
alla classica scrittura di Gauss-Seidel che aggiorna direttamente le componenti di
$x$: la caratteristica distintiva risiede nel fatto che il contributo dei termini
triangolari inferiori viene incorporato \emph{al volo} nella stessa iterazione, non
appena le relative componenti di $y$ risultano disponibili.

\begin{implbox}
Nel metodo \texttt{gauss\_seidel} implementato in Python, la matrice sparsa viene
convertita in formato denso una sola volta prima del ciclo principale:
\begin{center}
    \texttt{L = np.tril(self.A.toarray())}
\end{center}
All'inizio di ogni iterazione, si calcola il residuo globale
\texttt{r = self.b - self.A.dot(x)}. Un ciclo interno percorre poi le righe
$i = 0, \ldots, n-1$ per calcolare le componenti del vettore di correzione \texttt{y},
inizializzato a zero:
\begin{center}
    \texttt{s = r[i] - L[i, :i].dot(y[:i])} \\[2pt]
    \texttt{y[i] = s / L[i, i]}
\end{center}
Al termine del ciclo interno, l'iterata viene aggiornata con \texttt{x += y}. La
scelta di lavorare in formato denso e di implementare manualmente la sostituzione in
avanti è motivata da ragioni didattiche, evitando il ricorso a routine esterne di
\texttt{scipy.sparse.linalg}. Sebbene tale approccio comporti un maggiore utilizzo di
memoria rispetto a una gestione puramente sparsa, esso risulta pienamente sostenibile
per le dimensioni delle matrici presenti nel dataset di progetto.
\end{implbox}
...
che si annulla esattamente in $\widetilde{y} = A^{-1}b$.
...
\begin{enumerate}
    ...
    \item Anche se il residuo raggiungesse tale ordine di grandezza, l'errore effettivo sulla soluzione sarebbe amplificato dal numero di condizionamento: per una matrice con $Cond(A)$, un residuo relativo di $10^{-16}$ non garantisce affatto un errore sulla soluzione di $10^{-16}$, ma potenzialmente di $Cond(A) \cdot 10^{-16}$.
\end{enumerate}
La relazione tra residuo relativo ed errore relativo sulla soluzione è:
\begin{equation}
    \frac{\|x^* - x\|_2}{\|x^*\|_2} \leq Cond(A) \cdot \frac{\|Ax - b\|_2}{\|b\|_2}.
\end{equation}
...
\begin{itemize}
    \item \texttt{spa1.mtx}, \texttt{spa2.mtx} :matrici di dimensioni 1000x1000 e 3000x3000, con numeri di condizionamento rispettivamente 2.05e+03 e 1.41e+03, risultano piuttosto dense con densita ( percentuale degli elementi diversi da zero) rispettivamente 18,24\% e 18,14\%
    \item \texttt{vem1.mtx}, \texttt{vem2.mtx} : matrici di dimensioni 1681x1681 e 2601x2601 , con numeri di condizionamento rispettivamente 3.25e+02 e 5.07e+02, e densita molto bassa rispettivamente 0,47\% e 0.31\%
\end{itemize}
...
\begin{itemize}
    ...
    a zig-zag che lo porta ad oscillare attorno al minimo, probabilmente dovuto dall'condizionamento e dalla densita maggiore delle suddette matrici .
    Su \texttt{vem1} e \texttt{vem2}, si comporta invece in modo simile a
